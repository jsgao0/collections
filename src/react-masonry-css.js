...
class Masonry extends React.Component {
  ...
  itemsInColumns() {
    let currentColumnCount = this.state.columnCount;
    ...
    for (let i = 0; i < items.length; i++) {
      let columnIndex = i % currentColumnCount;
    ...
    if (!this.props.padEmptyColumn) {
      return itemsInColumns;
    }

    const itemCount = this.props.children.length;
    let columnOffset = 0;
    if (currentColumnCount > itemCount) {
      const diff = currentColumnCount - itemCount;
      if (diff % 2) {
        columnOffset = (diff + 1) / 2;
      } else {
        columnOffset = diff / 2;
      }
    }

    const padEmptyColumns = new Array(columnOffset).fill([]);

    return [...padEmptyColumns, ...itemsInColumns.filter(column => column), ...padEmptyColumns];
  }
